...
int main(void){
	/*==================== A single, two-word command can be parsed with varying whitespace around words ====================*/
	printf("\nA single, two-word command can be parsed with varying whitespace around words\n\n");
	struct pipeline* my_pipeline = pipeline_build("ls    cat\n");

	#ifdef MULTIPLE_PARSES //make check CFLAGS=-DMULTIPLE_PARSES
		pipeline_free(my_pipeline);
		my_pipeline = pipeline_build("ls    cat\n");
	# endif
	...
	TEST_ASSERT(strcmp("cat", my_pipeline->commands->command_args[1]) == 0);
